const emails = [
  ...
  const countCriminals = (emails) => {
      ...
      var removedR = 0
      var confirmedCOUK = 0
      var removedWorGShort = 0
      var confirmedWorGShort = 0
      var confirmedByDblT = 0
      var confirmedCriminals = []
      var confirmedInnocents = []
      console.log("length:" +allEmails.length)
    ...
    for(let i=0; i < allEmails.length; i++){
        ...
        //index of @ searched because last character of lastname is always before @
        const lnameLastLetterIndex = allEmails[i].indexOf("@") - 1
        const letter = allEmails[i].charAt(lnameLastLetterIndex)

        /////////////second rule//////////////////

        //last five chars from string
        const lastFive = allEmails[i].substr(allEmails[i].length - 5)
        //cuts everything out of string after @ character
        const fullName = allEmails[i].split('@')[0]
        ...
        var amountOfT = 0
        //checks every letter of fullname and makes sum of them
        for(let z = 0; z<fullName.length; z++){
            if(fullName[z].includes("t")){
                amountOfT ++
            }
        }
        ...
        if(letter === "r"){
            console.log("innocent because r: "+ allEmails[i])
            confirmedInnocents.push(allEmails[i])
            removedR++
        } 
        else if(lastFive === "co.uk" && fullName.includes("c")){
            console.log("criminal because co.uk and c in full name")
            confirmedCriminals.push(allEmails[i])
            confirmedCOUK++
        }
        else if(allEmails[i].includes("wonkaindustries") || allEmails[i].includes("gringottsbank")){
            //cuts everything out of string after . character
            const firstName = fullName.split('.')[0]
            
            if(firstName.length < 4){
                console.log("innocent because less than 4 character name "+ firstName)
                confirmedInnocents.push(allEmails[i])
                removedWorGShort ++
            } else {
                console.log("criminal because wonka or gringo and more than 4 character name "+ firstName)
                confirmedCriminals.push(allEmails[i])
                confirmedWorGShort ++
            }
        } 
        else if(amountOfT >= 2){
            console.log("criminal because double or more t:s: "+amountOfT)
            confirmedCriminals.push(allEmails[i])
            confirmedByDblT ++
        }
    }
    ...
    //info
    console.log("went through: " +allEmails.length+ " emails")
    //innocent
    console.log("confirmed innocent because r: "+removedR)
    console.log("confirmed innocent because wonka and gringo and shorter than 4: "+removedWorGShort)
    //criminal
    console.log("confirmed criminals because co.uk and c: "+confirmedCOUK)
    console.log("confirmed criminals because wonka and gringo and 4 or longer: "+confirmedWorGShort)
    console.log("confirmed criminals by more than 2 t:s: "+confirmedByDblT) 
    
    //criminals and innocents
    console.log("confirmed criminals: " + confirmedCriminals.length)
    console.log("confirmed innocents: " + confirmedInnocents.length)
    
    //check that all emails ran
    console.log("run amounts: " + runAmounts)

    // return the amount of criminals in the emails
    return confirmedCriminals.length;
  }

  console.log("there are: " + countCriminals(emails) + " criminals")
